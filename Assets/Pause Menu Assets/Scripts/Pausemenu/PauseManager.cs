       ...
       public  Terrain terrain;
        ...
        public void Update()
        {
           ...
           /* if (vidPanel.active == true)
			...
			if (mainPanel.active == true && SceneManager.GetActiveScene().buildIndex!=0)
            {
                pauseMenu.text = "Pause Menu";
            }

			if (Input.GetKeyDown(KeyCode.Escape) && mainPanel.active == false && SceneManager.GetActiveScene().buildIndex!=0)
			...
			else if(Input.GetKeyDown(KeyCode.Escape) && mainPanel.active == true &&SceneManager.GetActiveScene().buildIndex!=0 ) {
                Time.timeScale = timeScale;
				AudioListener.volume= beforeMaster;
                mainPanel.SetActive(false);
                vidPanel.SetActive(false);
